...
void contar_frequencias(FILE *arquivo, Frequencia *tabela)
{
    for (int i = 0; i < 256; i++)
    {
        tabela[i].byte = (BYTE)i;
        tabela[i].frequencia = 0;
    }
...
void compactar_arquivo(const char *entrada, const char *saida)
{
    ...
    Frequencia tabela_frequencias[256];
    contar_frequencias(in, tabela_frequencias);

    No *raiz = construir_arvore(tabela_frequencias);
    Codigo tabela[256] = {0};
    BYTE codigo[256];
    ...
    rewind(in);   // <<-- garante que vamos reler todo o arquivo
    fseek(out, 2, SEEK_SET);
    ...
    for (long i = 0; i < tamanho_total; i++)
{
    ...
    if (i == tamanho_total - 1)
        limite_bits = 8 - trash_bits;

    for (int j = 7; j >= 8 - limite_bits; j--)
    {
        int bit = (c >> j) & 1;
