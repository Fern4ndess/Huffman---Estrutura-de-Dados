...
int comparar_frequencia(void* a, void* b) {
    ...
    return (int)n1->frequencia - (int)n2->frequencia;
}
...
void serializar_arvore(NoHuffman* no, FILE* saida, unsigned short *tamanho) {
    ...
    if (!no->esquerda && !no->direita) {
        fputc('1', saida);
        (*tamanho)++;
        fputc((unsigned char)no->caractere, saida);
        (*tamanho)++;
    } 
    else {
        fputc('0', saida);
...
void verificar_header(const char* filename) {
    ...
    unsigned char header_bytes[2]; // antes, char
    ...
    unsigned short header_value = (unsigned short)(((unsigned char)header_bytes[0] << 8) | (unsigned char)header_bytes[1]);
    int trash_bits = (header_value >> 13) & 0x7;
    unsigned short tree_size = header_value & 0x1FFF;

    printf("\n=== VERIFICACAO DO HEADER ===\n");
    printf("Header em hexa: %02X %02X\n", header_bytes[0], header_bytes[1]);
    printf("Trash bits: %d\n", trash_bits);
    printf("Tamanho da Arvore: %d bytes\n", tree_size);
...
void compactar_arquivo(const char *input_filename, const char *output_filename) {
    ...
    for (size_t i = 0; codigo[i] != '\0'; i++) {
        buffer = (unsigned char)((buffer << 1) | (unsigned char)(codigo[i] == '1'));
        bits_preenchidos++;
    ...
    unsigned char trash_bits = 0;
    if (bits_preenchidos > 0) {
        trash_bits = (unsigned char)(8 - bits_preenchidos);
        buffer = (unsigned char)( buffer << trash_bits );
        fwrite(&buffer, 1, 1, out);
    }

    // Atualiza o header
    fseek(out, 0, SEEK_SET);
    unsigned short header_value = (unsigned short)(((unsigned short)trash_bits << 13) | (tamanho_arvore & 0x1FFF));
